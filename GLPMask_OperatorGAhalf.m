function [Offspring,outIndexList,chosengroups] = GLPMask_OperatorGAhalf(Parent1,Parent2,Parent1Mask,Parent2Mask, numberOfGroups)
...
%--------------------------------------------------------------------------
    ...
    MaskPool=Parent1Mask&Parent2Mask;
    ...
    Mask1Pool=zeros(N,D);
    Mask1Pool(~MaskPool)=Parent1Mask(~MaskPool);
    Mask2Pool=zeros(N,D);
    Mask2Pool(~MaskPool)=Parent2Mask(~MaskPool);
    Offspring(logical(Mask1Pool))=Parent1(logical(Mask1Pool));
    Offspring(logical(Mask2Pool))=Parent2(logical(Mask2Pool));
